reset && \
victory run web samples/google-search && \
victory run web samples/page-object-pattern && \
victory run web samples/find-elements && \
victory run web samples/google-search-with-sqlite && \
victory run web samples/frames && \
victory run web samples/frames_2 && \
victory run web samples/upload && \
victory run web samples/windows && \
victory run --headless web samples/google-search && \
victory run --headless web samples/page-object-pattern && \
victory run --headless web samples/find-elements && \
victory run --headless web samples/google-search-with-sqlite && \
victory run --headless web samples/frames && \
victory run --headless web samples/frames_2 && \
victory run --headless web samples/upload && \
victory run --headless web samples/windows
